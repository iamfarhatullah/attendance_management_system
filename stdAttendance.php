		...
		<div class="row">
          <div class="col-md-12">
            <div class="page-title">
              <h3><i class="fa fa-user-check"> </i> <?php echo $sub_name; ?><span></span></h3>
            </div>
            <hr>
          </div>
        </div>
...
if (mysqli_num_rows($resultDate)>0) {
	while ($rowDate = mysqli_fetch_assoc($resultDate)) {
		...
		echo "<th class='th' style='text-align: center; padding: 3px;'><p>".$day.'-'.$month.'<br>'.$year."</p></th>";
	}
}
...
WHERE att_groupID='$id' and tbl_students.std_rollno = '$rollno'
group by att_studentID";
$result = mysqli_query($conn, $sql);
$num = 1;
if (mysqli_num_rows($result)>0) {
	while ($rows = mysqli_fetch_assoc($result)) {
		...
		echo "<td style='text-align:center;'>
				<a href='stdAttendance.php?rollno=".$classno."' title='Edit'><span class='fa fa-edit'></span></a>
			</td>";
		echo "<tr>";
		$num++;
	}
...
<button style="position: fixed; bottom: 40px; background-color: transparent; color: blue; text-decoration: underline; border: none;" onclick="myFunction()">Print this page</button>
